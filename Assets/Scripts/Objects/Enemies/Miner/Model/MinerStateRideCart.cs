...
namespace Assets.Scripts.Objects.Enemies.Miner.Model
{
    public class MinerStateRideCart : IEStateAfterBorn
    {
        ...
        private async UniTask RideCart()
        {
            ...
            eM.MoveIgnoringStage(new Vector2(-10f, StageData.Instance.StageCenterPos.y) - eM.PA.Pos);

            float moveDistance = (StageData.Instance.StageCenterPos.x - eM.PA.Pos.x) * 2f;
            ...
            await GameObject.InstantiateAsync(bigCartPrefab, eM.PA.Pos + Vector2.up * 2f, Quaternion.identity);
            await UniTask.Delay(TimeSpan.FromSeconds(intervalSeconds), cancellationToken: eM.Token);
            await GameObject.InstantiateAsync(bigCartPrefab, eM.PA.Pos + Vector2.down * 2f, Quaternion.identity);
            await UniTask.Delay(TimeSpan.FromSeconds(intervalSeconds), cancellationToken: eM.Token);

            GameObject[] bigCart = await GameObject.InstantiateAsync(bigCartPrefab, eM.PA.Pos, Quaternion.identity);
